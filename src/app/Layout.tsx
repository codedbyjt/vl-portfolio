...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const isSharedAlbum = (location.pathname === '/photography' && searchParams.get('ref') === 'shared')
          ...
          <li key={label}>
            {external ? (
              <a
                ...
                onClick={onClick}
                className="block text-[14px] leading-7 tracking-wide text-gray-500 hover:text-gray-900 transition-colors"
              >
                {label}.
              </a>
            ) : (
              <button
                ...
                className={`block text-[14px] leading-7 tracking-wide transition-colors text-left w-full ${
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      {isSharedAlbum && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-white flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <button onClick={() => navigate('/')} className="hover:opacity-60 transition-opacity">
            <img src={publicAsset('/logo-tight.png')} alt="Vic Lentaigne" className="w-[160px] h-auto" />
          </button>
          {location.pathname === '/about' ? (
            ...
            <button onClick={() => navigate('/about?ref=album')}
              className="text-[12px] uppercase tracking-widest text-gray-500 hover:text-gray-900 transition-colors">
              About
            </button>
          )}
      ...
      {!isSharedAlbum && (
        ...
        >
          <img src={publicAsset('/logo-tight.png')} alt="Vic Lentaigne" className="w-full max-w-[212px] h-auto" />
        </button>
        <div className="px-8">
          <NavLinks />
        </div>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <button onClick={() => navigate('/')} className="hover:opacity-60 transition-opacity">
          <img src={publicAsset('/logo-tight.png')} alt="Vic Lentaigne" className="w-[170px] h-auto" />
        </button>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            >
              <button onClick={() => setMobileOpen(false)} className="absolute top-4 right-4" aria-label="Close">
                <X size={20} className="text-gray-600" />
              </button>
              <button onClick={() => { navigate('/'); setMobileOpen(false); }} className="mb-10 block hover:opacity-60 transition-opacity">
                <img src={publicAsset('/logo-tight.png')} alt="Vic Lentaigne" className="w-[170px] h-auto" />
              </button>
      ...
      {/* ── Page content ── */}
      {isSharedAlbum ? (
        <main className="min-h-screen pt-[57px]">
          {children}
        </main>
      ) : isHome ? (
